...
const mediaId = async (imgPath) =>{
    ...
    form.append('access_token', `${PAT}`);
        ...
        `https://graph.facebook.com/v23.0/${PID}/photos?access_token=${PAT}`,form,{headers}
    );
    const { id } = res.data;
    return id;
    }
...
cron.schedule('*/2 * * * *', () => {
    ...
    (async () =>{
                try {
                    const feed = await parser.parseURL('https://saurahaonline.com/rss');
                    ...
                    for (let item of feed.items) {
                        console.log(feed.items.length)
                              if(!posted.has(item.link)){
                                ...
                                let dataFinal = item['content:encoded'].replace(/<\/?[^>]+(>|$)/g, '').trim();
                                const imageUrl = await extractImageFromArticle(item.link);  // cheerio
